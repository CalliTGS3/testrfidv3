...
namespace MFRC522 {
    ...
    let SPI_SS   = DigitalPin.P0
    let SPI_MOSI = DigitalPin.C9
    let SPI_MISO = DigitalPin.C8
    let SPI_SCK  = DigitalPin.C7

    let antenna_gain = 0x04
    const RFIDEventID = 3102
    let LastID = 0
    ...
    function SPI_Write (adr: number, val: number) {
        pins.digitalWritePin(SPI_SS, 0)
        pins.spiWrite((adr << 1) & 0x7E)
        pins.spiWrite(val)
        pins.digitalWritePin(SPI_SS, 1)
    }
     ...
     * Initial setup
     */
    //% block="Initialize MFRC522 Module with pins MOSI %mosi MISO %miso SCK %sck SS %ss"
    //% weight=100
   export function Init(mosi: DigitalPin, miso: DigitalPin, sck: DigitalPin, ss: DigitalPin) {
       ...
       SPI_Write(0x12, 0x00)        // TxModeReg
       SPI_Write(0x13, 0x00)        // RxModeReg
       SPI_Write(0x24, 0x26)        // ModWidthReg
       SPI_Write(0x2A, 0x80)        // TModeReg
       SPI_Write(0x2B, 0xA9)        // TPrescalerReg
       SPI_Write(0x2C, 0x03)        // TReloadRegH
       SPI_Write(0x2D, 0xE8)        // TReloadRegL
       SPI_Write(0x15, 0x40)        // TxASKReg
       SPI_Write(0x11, 0x3D)        // ModeReg
       
       // SPI_Write(0x25, antenna_gain << 4)
...
*/
   ...
   function getIDInternal() {
    ...
    //% blockId=onRFIDEvent block="wenn RFID Chip präsentiert"
    export function onRFIDEvent(handler: () => void) {
        control.onEvent(RFIDEventID, EventBusValue.MICROBIT_EVT_ANY, handler);
...
//                    getIDInternal()
                    if (id != LastID) {  
                        LastID = id
//                        serial.writeValue("Event", id)
                        control.raiseEvent(RFIDEventID, 0)
                        control.clearInterval(0, control.IntervalMode.Interval)
                    }
                } else
                {
                    LastID = 0
                }
//                serial.writeValue("LastID", LastID)
                basic.pause(10);
            }
        })
    }
